    ...
    >
      ...
      >
        ...
        <p class="text-slate-500 dark:text-slate-400">
          Shorten long links and track them.
        </p>
      </header>
      ...
      <div class="flex-1 p-6 flex flex-col items-center gap-8">
        <!-- Creation Card -->
        <div
          class="w-full max-w-2xl bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 md:p-8"
        >
          <div class="space-y-6">
            <div>
              <label
                class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2"
                >Long URL <span class="text-red-500">*</span></label
              >
              <input
                type="url"
                [(ngModel)]="url"
            ...
            @if (error()) {
              <div
                class="p-3 bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/20 text-red-600 dark:text-red-400 rounded-lg text-sm flex items-center gap-2"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="16"
        ...
        @if (result()) {
          <div
            class="w-full max-w-2xl bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-indigo-100 dark:border-indigo-900/50 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500"
          >
            <div
              class="p-6 md:p-8 flex flex-col md:flex-row gap-8 items-center"
            >
              <!-- QR Code -->
              <div
                class="flex-none bg-white p-2 rounded-xl border border-slate-100 shadow-sm"
              >
              ...
              <div class="flex-1 w-full text-center md:text-left space-y-4">
                <div>
                  ...
                  <h2
                    class="text-2xl md:text-3xl font-bold text-indigo-600 dark:text-indigo-400 truncate tracking-tight"
                  >
                    {{ result()?.shortUrl }}
                  </h2>
                </div>
        ...
        @if (history().length > 0) {
          <div class="w-full max-w-2xl mt-4">
            <h3
              class="text-lg font-bold text-slate-800 dark:text-slate-100 mb-4 px-1"
            >
              Recent Links
            </h3>
            <div
            ...
            >
              <div class="overflow-x-auto">
                <table class="w-full text-left">
                  <thead
                    class="bg-slate-50 dark:bg-slate-700/50 border-b border-slate-100 dark:border-slate-700"
                  >
                    <tr>
                      <th
                      ...
                      <th
                        class="px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider w-10"
                      >
                        Action
                      </th>
                    </tr>
                  ...
                  >
                    @for (item of history(); track item.slug) {
                      ...
                      >
                        <td
                          class="px-6 py-4 text-sm text-slate-600 dark:text-slate-300 max-w-[200px] truncate"
                          [title]="item.originalUrl"
                        >
                          {{ item.originalUrl }}
                        </td>
                        <td
                          class="px-6 py-4 text-sm font-medium text-indigo-600 dark:text-indigo-400"
                        >
                          {{ item.shortUrl }}
                        </td>
...
export class ShortenerComponent {
  ...
  shorten() {
    ...
    this.shortenerService.shortenUrl(this.url(), this.customSlug()).subscribe({
      next: (response) => {
