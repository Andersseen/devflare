...
import { isExternalLink, type Tool } from './shell-navigation';
...
@Component({
  ...
  template: `
    <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
      @for (tool of tools(); track tool.link) {
        @if (isExternal(tool.link)) {
          <a
            ...
            class="group block"
          >
            <ng-container *ngTemplateOutlet="cardBody; context: { tool }" />
          </a>
        } @else {
      ...
      <ng-template #cardBody let-tool="tool">
        <volt-card
