  ...
  template: `
    <div class="space-y-6">
      <div>
        ...
        <p class="text-muted-foreground mt-1">Preview how your pages appear on Google, Twitter, and Facebook</p>
      </div>
          ...
          <volt-card>
            <volt-card-content class="space-y-4">
              <div>
                ...
                />
                @if (title().length > 70) {
                  <p class="text-xs text-red-500 mt-1">Google will likely truncate this title.</p>
                }
              </div>
              ...
              <volt-button variant="outline" class="w-full" (click)="copyHtml()">
                Copy Meta Tags
              </volt-button>
            </volt-card-content>
          </volt-card>
        ...
        <div class="space-y-6">
          <volt-tabs [(value)]="activePreview">
            ...
            <volt-tabs-content value="google">
              <div class="bg-white dark:bg-card p-4 rounded-lg border border-border font-sans">
                <div class="flex items-center gap-3 mb-1">
                  ...
                  <div class="text-sm">
                    <div class="text-foreground font-medium truncate max-w-[200px]">{{ getDomain() }}</div>
                    <div class="text-muted-foreground text-xs truncate max-w-[250px]">{{ url() }}</div>
                  </div>
                </div>
                <a href="#" class="block text-[#1a0dab] dark:text-[#8ab4f8] text-xl font-medium hover:underline truncate mb-1 leading-snug">
                  {{ title() || 'Page Title' }}
                </a>
                <div class="text-sm text-[#4d5156] dark:text-muted-foreground leading-relaxed line-clamp-2">
            ...
            <volt-tabs-content value="twitter">
              <div class="bg-white dark:bg-card rounded-xl border border-border overflow-hidden font-sans cursor-pointer hover:bg-accent/30 transition-colors">
                ...
                <div class="p-4">
                  <div class="text-muted-foreground text-xs uppercase mb-0.5 truncate">{{ getDomain().toUpperCase() }}</div>
                  <div class="text-foreground font-bold mb-1 truncate leading-tight">{{ title() || 'Page Title' }}</div>
                  <div class="text-muted-foreground text-sm line-clamp-2 leading-snug">{{ description() || 'Description goes here.' }}</div>
                </div>
              </div>
            </volt-tabs-content>
            ...
            <volt-tabs-content value="facebook">
              <div class="bg-[#F0F2F5] dark:bg-muted/30 p-3 rounded-lg">
                <div class="bg-white dark:bg-card border border-border overflow-hidden">
                  ...
                  <div class="p-3 bg-[#F2F3F5] dark:bg-muted/50 border-t border-border">
                    <div class="text-muted-foreground text-xs uppercase truncate mb-0.5">{{ getDomain().toUpperCase() }}</div>
                    <div class="text-foreground font-bold text-base truncate mb-0.5 leading-tight">{{ title() }}</div>
                    <div class="text-muted-foreground text-sm line-clamp-1 leading-snug">{{ description() }}</div>
                  </div>
                </div>
              </div>
            </volt-tabs-content>
              ...
              <div class="flex gap-3">
                ...
                <div class="flex-1">
                  <div class="flex gap-2 mb-1">
                    <span class="text-xs font-bold text-foreground">DevFlare</span>
