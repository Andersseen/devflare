    ...
    <div class="space-y-6">
      <div>
        ...
        <p class="text-muted-foreground mt-1">
          Smart compression with WebWorkers. Reduce file size without
          ...
          <volt-card>
            ...
            <volt-card-content class="space-y-4">
              <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                  <label for="quality" class="text-sm font-medium block mb-2"
                  ...
                  <input
                    ...
                    [value]="quality()"
                    (change)="
                      quality.set(+$any($event).target.value); processImage()
                    "
                    class="w-full"
                  />
                </div>
                <div>
                  <label for="maxWidth" class="text-sm font-medium block mb-2"
                    >Max Width:
                    {{ maxWidth() > 0 ? maxWidth() + 'px' : 'Original' }}</label
                  >
                  <input
                    id="maxWidth"
                    type="range"
                    ...
                    [value]="maxWidth()"
                    (change)="
                      maxWidth.set(+$any($event).target.value); processImage()
                    "
                    class="w-full"
                  />
                </div>
                <div>
                  <label for="format" class="text-sm font-medium block mb-2"
                    >Format</label
                  >
                  <select
                    id="format"
...
export default class ImageCompressorPage {
  ...
  savings = computed(() =>
    this.#imageCompressorService.getSavingsPercent(
      this.originalSize(),
      this.compressedSize(),
    ),
  );
