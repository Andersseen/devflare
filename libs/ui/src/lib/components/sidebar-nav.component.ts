...
import { NavSection as NavbarSection } from './navbar.component';
...
@Component({
  ...
  template: `
    <nav class="p-4 space-y-1">
      @for (group of displayedGroups(); track group.title) {
        @if (group.title) {
...
export class SidebarNavComponent {
  ...
  private deploymentGroups: NavGroup[] = [
    ...
    },
    {
      title: 'Storage',
      items: [
        { title: 'Buckets', path: '/storage/explorer', iconName: 'database' },
      ],
    },
  ];
  ...
  displayedGroups = computed(() => {
    return this.activeSectionInput() === 'deployment'
